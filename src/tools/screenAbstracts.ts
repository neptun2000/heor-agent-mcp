...
const PICOCriteriaSchema = z.object({
  population: z
    .string()
    .min(1)
    .describe("Target population (e.g., 'adults with type 2 diabetes')"),
  intervention: z
    .string()
    .min(1)
    .describe("Intervention of interest (e.g., 'semaglutide')"),
  comparator: z
    .string()
    .optional()
    .describe("Comparator (e.g., 'placebo', 'sitagliptin')"),
  outcomes: z
    .array(z.string())
    .optional()
    .describe(
      "Outcomes of interest (e.g., ['HbA1c', 'weight', 'cardiovascular events'])",
    ),
});
...
const ScreenAbstractsSchema = z.object({
  ...
  exclude_study_types: z
    ...
    .optional()
    .describe(
      "Study types to exclude (e.g., ['case_report', 'editorial', 'letter', 'commentary'])",
    ),
...
function classifyStudyDesign(
  ...
  if (
    ...
    combined.includes("prospective") ||
    combined.includes("longitudinal")
  )
    return "Cohort Study";
  if (
    combined.includes("retrospective") ||
    combined.includes("database") ||
    combined.includes("claims") ||
  ...
  if (
    combined.includes("cost-effectiveness") ||
    combined.includes("cost effectiveness") ||
    combined.includes("economic") ||
    combined.includes("budget impact") ||
    combined.includes("icer") ||
    combined.includes("qaly")
  )
    return "Health Economic Study";
  if (
    combined.includes("guideline") ||
    combined.includes("recommendation") ||
    combined.includes("consensus")
  )
    return "Guideline / Consensus";
  if (
    combined.includes("hta") ||
    combined.includes("appraisal") ||
    combined.includes("reimbursement") ||
    combined.includes("technology assessment")
  )
    return "HTA Report";
...
function getDesignScore(designClass: string): number {
  if (designClass.includes("Systematic") || designClass.includes("Meta"))
    return 5;
...
function computeRelevanceScore(
  ...
  // Population match
  const popTerms = criteria.population
    .toLowerCase()
    .split(/\s+/)
    .filter((t) => t.length > 3);
  ...
  // Intervention match
  const intTerms = criteria.intervention
    .toLowerCase()
    .split(/[\s,]+/)
    .filter((t) => t.length > 2);
  ...
  if (criteria.comparator) {
    const compTerms = criteria.comparator
      .toLowerCase()
      .split(/[\s,]+/)
      .filter((t) => t.length > 2);
...
export async function handleScreenAbstracts(
  ...
  for (const result of params.results as LiteratureResult[]) {
    ...
    if (params.min_year) {
      ...
      if (year > 0 && year < params.min_year) {
        screeningResults.push({
          ...
          decision: "exclude",
          reasons: [
            `Published ${year}, before minimum year ${params.min_year}`,
          ],
          pico_match: {
            population: false,
            intervention: false,
    ...
    const typeKey =
      result.study_type?.toLowerCase().replace(/[\s-]/g, "_") ?? "";
    ...
    if (score >= threshold + 0.2) {
      ...
      decision = "exclude";
      reasons.push(
        `Relevance score ${score.toFixed(2)} below threshold ${threshold}`,
      );
    }
  ...
  const picoStr = [
    ...
    params.criteria.outcomes
      ? `**O:** ${params.criteria.outcomes.join(", ")}`
      : null,
  ]
    .filter(Boolean)
    .join(" | ");
  ...
  if (excluded.length > 0) {
    ...
    lines.push(
      `<details><summary>Click to expand excluded studies</summary>\n`,
    );
    lines.push(`| Study | Reason |`);
    lines.push(`|-------|--------|`);
    excluded.forEach((r) => {
      lines.push(
        `| ${r.title.slice(0, 70)} | ${r.reasons[0] ?? "Low relevance"} |`,
      );
    });
    lines.push(`\n</details>\n`);
  }
      ...
      criteria: {
        ...
        properties: {
          population: {
            type: "string",
            description:
              "Target population (e.g., 'adults with type 2 diabetes')",
          },
          intervention: {
            type: "string",
          ...
          comparator: {
            type: "string",
            description:
              "Comparator (e.g., 'placebo', 'sitagliptin'). Optional.",
          },
          outcomes: {
            type: "array",
            items: { type: "string" },
            description:
              "Outcomes of interest (e.g., ['HbA1c', 'weight loss', 'MACE']). Optional.",
          },
        },
