...
describe("deriveFromAudit", () => {
  it("a run with validate_links + sources + methodology scores green on 1/2/6", () => {
    let audit = createAuditRecord("literature.search", {}, "markdown", "PRISMA search");
    audit = addToolCall(audit, { name: "utils.validate_links", ms: 100, outcome: "ok" });
  ...
  it("sources but no validate_links → citation partial", () => {
    let audit = createAuditRecord("literature.search", {}, "markdown", "search");
    audit = addSource(audit, {
      source: "pubmed",
      query_sent: "x",
      results_returned: 1,
  ...
  it("no sources at all → citation red", () => {
    const audit = createAuditRecord("models.cost_effectiveness", {}, "markdown", "model");
    expect(deriveFromAudit(audit).citation_validation).toBe("red");
  });
});
